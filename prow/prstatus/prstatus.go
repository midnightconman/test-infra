...
type PullRequestQueryHandler interface {
	...
	BotName(*github.Client) (*github.User, error)
}
...
func (da *DashboardAgent) HandlePrStatus(queryHandler PullRequestQueryHandler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if ok && token.Valid() {
			...
			var err error
			user, err = queryHandler.BotName(githubClient)
			if err != nil {
...
func (da *DashboardAgent) BotName(client *github.Client) (*github.User, error) {
	user, err := client.BotName()
	if err != nil {
		return nil, err
	}

	return &github.User{Login: user}, nil
}
